...
let wordsList = function(str, subStr) {
    ...
    let arr = str.replace(re, '').toLowerCase().split(' ').filter((arrItem) => arrItem.indexOf(subStr) > -1);
    let mySet = new Set;
    arr.forEach((arrItem) => {
        mySet.add(arrItem);
    });
...
let car = new Car(1800, 'Lacetti', 'Chevrolet', 2006);
let carInfinite = new Car(2000, 'RX3000', 'Infinite', 2019);
